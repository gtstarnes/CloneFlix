...
const Show = ({show} : {show: Show}) => {
  ...
  return (
    <div className="w-[280px] sm:w-[280px] md:w-[360px] lg:w-[440px] inline-block cursor-pointer relative mx-2">
        <img className="w-full h-auto block" src={`https://image.tmdb.org/t/p/w500/${show.backdrop_path}`} alt={show.name} />
        <div className="absolute top-0 left-0 w-full h-full hover:bg-black/60 opacity-0 hover:opacity-100 break-all">
            <p className="white-space-normal text-xs sm:text-sm md:text-md lg:text-lg font-bold flex flex-wrap justify-center items-center h-full text-center">{show.name}</p>
            <p>
